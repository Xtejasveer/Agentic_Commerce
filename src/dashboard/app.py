...
def _background_seed_check():
    """Seed catalog in background without delaying server port binding."""
    import time
    time.sleep(1)
    try:
        ...
        with SessionLocal() as db:
            if db.query(ProductRecord).count() == 0:
                logging.getLogger("auto_seed").info("Database catalog is empty. Auto-seeding 50 products...")
                for p in PRODUCTS:
                ...
                db.commit()
                logging.getLogger("auto_seed").info("50 products seeded into PostgreSQL.")

            try:
                if vector_db.collection.count() == 0:
                    logging.getLogger("auto_seed").info("ChromaDB vector collection is empty. Seeding embeddings...")
                    vector_db.add_products(PRODUCTS)
                    logging.getLogger("auto_seed").info("50 products seeded into ChromaDB.")
            except Exception as ve:
                logging.getLogger("auto_seed").warning(f"ChromaDB auto-seed notice: {ve}")
        ...
        logging.getLogger("auto_seed").warning(f"Background seed error: {e}")

threading.Thread(target=_background_seed_check, daemon=True).start()
